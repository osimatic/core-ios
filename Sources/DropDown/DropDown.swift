...
open class DropDown: UITextField {
	...
	public var optionArray = [DropDownItem]() {
		didSet {
		...
		didSet {
			if searchText == "" {
				...
				dataArray = optionArray.filter {
					searchFilter(text: $0.getLabel(), searchText: searchText)
				}.map({ $0.getLabel() });
			}
			reSizeTable()
			selectedIndex = nil
	...
	public func setSelectedIndex(_ selectedIndex: Int) -> Void {
	...
	public func getSelectedItem() -> DropDownItem? {
		if let selectedIndex = self.selectedIndex, selectedIndex >= 0 && selectedIndex < self.optionArray.count {
			return self.optionArray[selectedIndex];
		}
		return nil;
	}
...
extension DropDown: UITableViewDelegate {
	public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		selectedIndex = isSearchEnable ? (optionArray.firstIndex(where: { $0.getLabel() == selectedText }) ?? currentIndex) : currentIndex // Correct Index For Searched Text
		...
		if let selected = optionArray.firstIndex(where: { $0.getLabel() == selectedText }) {
